...
app.post("/agent",async(req,res)=>{
 const messages: ChatCompletionMessageParam[]=[{ role:"user", content:req.body.message }];

 const first = await client.chat.completions.create({ model: 'qwen3.7-plus-2026-05-26', messages, tools});
 ...
 const second = await client.chat.completions.create({ model: 'qwen3.7-plus-2026-05-26', messages, tools});
 ...
 if(response2.finish_reason==="tool_calls"){
   ...
   * 让模型把工具结果转换成人话。
   */
  const final = await client.chat.completions.create({ model: 'qwen3.7-plus-2026-05-26', messages });
